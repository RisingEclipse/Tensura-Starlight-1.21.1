...
public class convergenceBornRace extends lesserConvergenceBornRace {
    public convergenceBornRace(Difficulty difficulty) {
        super(difficulty);
    }

    public convergenceBornRace() {
    ...
    public RaceConfig.Default getDefaultConfig() {
        return (ConfigRegistry.getConfig(aspectBornRaceConfig.class)).convergenceBorn;
    }
    ...
    public List<ManasRace> getNextEvolutions(ManasRaceInstance instance, LivingEntity entity) {
        return List.of(StarlightRaces.GREATER_CONVERGENCE_BORN.get());
    }

    public Map<EvolutionRequirement, Float> getEvolutionRequirements(ManasRaceInstance previous, LivingEntity entity) {
        return Map.of(new EvolutionRequirement.EPRequirement((ConfigRegistry.getConfig(aspectBornRaceConfig.class)).convergenceBorn.epRequirement), 100.0F);
    }

    public List<ManasSkill> getIntrinsicSkills(ManasRaceInstance instance, LivingEntity entity) {
        return ConfigRegistry.getConfig(aspectBornRaceConfig.class)
                .convergenceBorn.convergenceBornIntrinsics.stream()
